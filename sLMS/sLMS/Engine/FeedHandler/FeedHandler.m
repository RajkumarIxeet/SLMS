...
-(void)setCommentOnUpdate:(NSString*)updateId  AndCommentText:(NSString*)txtComment success:(void (^)(BOOL logoutValue))success failure:(void (^)(NSError *error))failure{
    ...
    NSDictionary *parameters = @{@"userName":[AppSingleton sharedInstance].userDetail.userEmail,@"updateId":updateId,@"commentText":txtComment                                 };
    
    [manager POST:CMT_ON_UPDATE_URL parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
-(void)setLikeOnUpdate:(NSString*)updateId  success:(void (^)(BOOL logoutValue))success failure:(void (^)(NSError *error))failure
{
    ...
    [manager GET:LIKE_ON_UPDATE_URL(useremail,updateId) parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
